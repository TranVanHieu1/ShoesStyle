...
public class ChangePassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String confirmPass = request.getParameter("newPass_confirm");
        RequestDispatcher rd = request.getRequestDispatcher("change-password.jsp");
        RegistrationDAO dao = new RegistrationDAO();
        if (oldPass.equalsIgnoreCase(acc.getStrPassword())) {
            if (newPass.equals(confirmPass)){
                try {
                    if (dao.changePass(acc.getStrUsername(), newPass)) {
                        acc.setStrPassword(newPass);
                ...
                request.setAttribute("UpdateNoti", "confirmNotMatch");
            }
        }
        else {
            request.setAttribute("UpdateNoti", "oldPassNotMatch");
        }
